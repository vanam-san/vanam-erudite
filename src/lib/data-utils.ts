...
async function getBlogCollection(): Promise<CollectionEntry<'blog'>[]> {
  if (!postsCache) {
    try {
      postsCache = await getCollection('blog')
    } catch (error) {
      console.error('Failed to fetch blog collection:', error)
      postsCache = []
    }
  }
  return postsCache
...
export function getAllPosts(): Promise<CollectionEntry<'blog'>[]> {
  return getPosts({ excludeSubposts: true })
}

export function getAllPostsAndSubposts(): Promise<CollectionEntry<'blog'>[]> {
  return getPosts({ excludeSubposts: false })
}

export async function getPosts({
  excludeSubposts = true,
}: { excludeSubposts?: boolean } = {}): Promise<CollectionEntry<'blog'>[]> {
  const posts = await getBlogCollection()
  return posts
    .filter(
      (post) => !post.data.draft && (!excludeSubposts || !isSubpost(post.id)),
    )
...
export async function getAllProjects(): Promise<CollectionEntry<'projects'>[]> {
  try {
    ...
    console.error('Failed to fetch projects collection:', error)
    return []
  }
}
...
export async function getAdjacentPosts(currentId: string): Promise<{
  ...
  if (isSubpost(currentId)) {
    ...
    const allPostsIncludingSubposts = await getAllPostsAndSubposts()
    const subposts = allPostsIncludingSubposts
      .filter(
        (post) =>
          isSubpost(post.id) &&
...
export async function parseAuthors(authorIds: string[] = []) {
  ...
  return authorIds.map((id) => {
    ...
    return {
      ...
      avatar: author?.data?.avatar || '/static/pic.png',
      isRegistered: !!author,
    }
  })
}
...
export async function getCombinedReadingTime(
  postId: string,
): Promise<ReadingTimeInfo> {
  const post = await getPostById(postId)
  if (!post)
    return { readingTime: readingTime(0), wordCount: wordCountDisplay(0) }
  ...
  return {
    readingTime: readingTime(totalWords),
    wordCount: wordCountDisplay(totalWords),
  }
}
...
export async function getPostReadingTime(
  ...
  return {
    ...
    wordCount: wordCountDisplay(wordCount),
  }
}
